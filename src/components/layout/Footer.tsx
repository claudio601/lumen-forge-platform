          ...
          <footer className="gradient-dark text-muted mt-16">
                <div className="container py-12">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                                  ...
                                  <div>
                                              <h4 className="font-semibold text-background mb-3 text-sm uppercase tracking-wider">Catalogo</h4>h4>
                            ...
                            <Link key={cat.id} to={`/catalogo/${cat.slug}`} className="flex items-center justify-between text-sm text-background/60 hover:text-primary transition-colors">
                                              ...
                                              <span className="text-[10px] text-background/30">{cat.productCount}</span>span>
                            </Link>Link>
                          ))}
                                                            <Link to="/catalogo" className="block text-sm text-primary hover:underline mt-2">Ver todas las categorias</Link>Link>
                                              </nav>nav>
                                  </div>div>
                                  <div>
                                              <h4 className="font-semibold text-background mb-3 text-sm uppercase tracking-wider">Empresa</h4>h4>
                                              <nav className="space-y-1.5">
                                                {[
            ...
            { label: 'Politica de privacidad', to: '/' },
                                            ...
                                            <Link key={label} to={to} className="block text-sm text-background/60 hover:text-primary transition-colors">{label}</Link>Link>
                                          ))}
                                              </nav>nav>
                                  </div>div>
                                  <div>
                                              <h4 className="font-semibold text-background mb-3 text-sm uppercase tracking-wider">Contacto</h4>h4>
                                              ...
                                              <div className="mt-5 p-3 bg-background/5 rounded-lg border border-background/10">
                                                            <p className="text-xs text-background/50 leading-relaxed">Lunes a viernes 9:00-18:00 hrs.<br />Santiago, Chile</p>p>
                                              </div>div>
                                  </div>div>
                        </div>div>
                        <div className="border-t border-background/10 mt-8 pt-6 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-background/40">
                                  <span>© {new Date().getFullYear()} eLIGHTS - Todos los derechos reservados</span>span>
                                  <span>Iluminacion al alcance de tus proyectos.</span>span>
                        </div>div>
                </div>div>
          </footer>footer>
        );
