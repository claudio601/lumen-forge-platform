    ...
    return (
          <div className="group border rounded-xl overflow-hidden hover:shadow-product-hover transition-all bg-card">
                ...
                <div className="p-4 space-y-2">
                        <p className="text-[10px] text-muted-foreground font-mono">{product.sku}</p>p>
                        ...
                        <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-[11px] text-muted-foreground">
                          {product.watts > 0 && <span>{product.watts}W</span>span>}
                          {product.lumens > 0 && <span>{product.lumens.toLocaleString('es-CL')}lm</span>span>}
                          {product.kelvin > 0 && <span>{product.kelvin}K</span>span>}
                          {product.beamAngle && <span>{product.beamAngle}deg</span>span>}
                        </div>div>
                        <div className="flex items-baseline gap-1.5">
                                  <p className="text-lg font-bold text-foreground">{formatDisplayPrice(product.price)}</p>p>
                                  <span
