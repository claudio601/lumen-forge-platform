  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      sendToPipedrive(payload).catch(err => {
        console.warn('[Pipedrive] Error inesperado:', err);
      });
      // GA4: evento de conversion con parametros utiles
      sendEvent('instalacion_form_submit_success', {
        tipoProyecto: form.tipoProyecto,
        comuna: form.comuna.trim(),
      });
      setStatus('success');
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-16 text-center">
        ...
        <p className="text-gray-500 max-w-sm text-sm leading-relaxed">
          Nuestro equipo revisara tu solicitud y te contactara en menos de 24 horas habiles con una propuesta personalizada.
        </p>
        <button
  ...
  return (
    <form onSubmit={handleSubmit} noValidate>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div>
          <label htmlFor="ilf-nombre" className={labelClass}>Nombre completo <span className="text-red-500">*</span></label>
          <input id="ilf-nombre" name="nombre" type="text" value={form.nombre} onChange={handleChange} placeholder="Juan Perez" className={inputClass} autoComplete="name" />
        </div>
        <div>
          <label htmlFor="ilf-telefono" className={labelClass}>Telefono <span className="text-red-500">*</span></label>
          <input id="ilf-telefono" name="telefono" type="tel" value={form.telefono} onChange={handleChange} placeholder="+56 9 XXXX XXXX" className={inputClass} autoComplete="tel" />
        </div>
        <div>
          <label htmlFor="ilf-email" className={labelClass}>Correo electronico <span className="text-red-500">*</span></label>
          <input id="ilf-email" name="email" type="email" value={form.email} onChange={handleChange} placeholder="correo@ejemplo.com" className={inputClass} autoComplete="email" />
        </div>
        <div>
          <label htmlFor="ilf-comuna" className={labelClass}>Comuna <span className="text-red-500">*</span></label>
          <input id="ilf-comuna" name="comuna" type="text" value={form.comuna} onChange={handleChange} placeholder="Ej: Las Condes, Maipu, Quilicura..." className={inputClass} />
        </div>
        <div className="sm:col-span-2">
          <label htmlFor="ilf-tipoProyecto" className={labelClass}>Tipo de proyecto <span className="text-red-500">*</span></label>
          <select id="ilf-tipoProyecto" name="tipoProyecto" value={form.tipoProyecto} onChange={handleChange} className={inputClass}>
            <option value="">Selecciona el tipo de proyecto...</option>
            {TIPOS_PROYECTO.map(t => (<option key={t} value={t}>{t}</option>))}
          </select>
        </div>
        <div className="sm:col-span-2">
          <label htmlFor="ilf-descripcion" className={labelClass}>Descripcion breve del proyecto <span className="text-red-500">*</span></label>
          <textarea id="ilf-descripcion" name="descripcion" value={form.descripcion} onChange={handleChange} rows={3} placeholder="Cuentanos el espacio, cantidad de puntos de luz aproximados, o cualquier detalle relevante." className={inputClass + ' resize-none'} />
        </div>
        <div>
          <label htmlFor="ilf-tipoCliente" className={labelClass}>Tipo de cliente <span className="text-xs font-normal text-gray-400">(opcional)</span></label>
          <select id="ilf-tipoCliente" name="tipoCliente" value={form.tipoCliente} onChange={handleChange} className={inputClass}>
            {TIPOS_CLIENTE.map(({ value, label }) => (<option key={value} value={value}>{label}</option>))}
          </select>
        </div>
        <div>
          <label htmlFor="ilf-preferenciaContacto" className={labelClass}>Preferencia de contacto <span className="text-xs font-normal text-gray-400">(opcional)</span></label>
          <select id="ilf-preferenciaContacto" name="preferenciaContacto" value={form.preferenciaContacto} onChange={handleChange} className={inputClass}>
            {PREFS_CONTACTO.map(({ value, label }) => (<option key={value} value={value}>{label}</option>))}
          </select>
        </div>
        <div className="sm:col-span-2">
          <label className="flex items-start gap-3 cursor-pointer select-none">
            <input type="checkbox" name="aceptaContacto" checked={form.aceptaContacto} onChange={handleChange} className="mt-0.5 h-4 w-4 flex-shrink-0 rounded border-gray-300 accent-purple-600 cursor-pointer" />
            <span className="text-sm text-gray-600 leading-snug">
              Acepto que eLIGHTS me contacte para responder a esta solicitud. No compartiremos tus datos con terceros.{' '}
      ...
      <div className="mt-8">
        ...
        >
          {status === 'sending' ? (
            ...
            <><Send className="h-5 w-5" /> SOLICITAR EVALUACION</>
          )}
        </Button>
