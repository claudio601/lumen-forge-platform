...
const RequestOrderPage = () => {
      ...
      const handleSubmit = async (e: React.FormEvent) => {
              ...
              // Fix 2: telefono validado por separado — phone nunca se duplica en missing[]
              const phoneEmpty = !form.phone.trim();
              const phoneDigits = form.phone.replace(/\D/g, '');
              const phoneInvalid = !phoneEmpty && phoneDigits.length < 8;
              ...
              try {
                        ...
                        setStatus('error');
                        setErrorMsg(
                                    'No pudimos enviar tu solicitud. Por favor intentalo de nuevo o escribenos por WhatsApp.'
                                  );
                        window.scrollTo({ top: 0, behavior: 'smooth' });
              }
      };
                    ...
                    <div className="border rounded-xl overflow-hidden mb-8">
                        ...
                        {items.map((item) => {
                            ...
                            const priceModeLabel =
                                            item.priceMode === 'neto' ? 'Precio neto (B2B)' : 'Precio con IVA (B2C)';
