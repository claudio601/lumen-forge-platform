...
import {
  ClipboardList, ArrowLeft, CheckCircle2, AlertCircle,
  Loader2, Minus, Plus, Trash2, Send,
} from 'lucide-react';
...
// ── GAS Relay (igual que QuoteCartPage y InstallationLeadForm) ────────────────
const GAS_URL =
  'https://script.google.com/macros/s/AKfycbwn2Qv3nJsNrUfBvzdpB9X70NmQfAVXgBKVw8bdmG-CXMXGsL-2IUcJaKX0mpO4kNwfOw/exec';

async function sendGasEmail(payload: RequestOrderPayload, ref: string): Promise<void> {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-5 py-20 text-center max-w-md mx-auto">
      ...
      <p className="text-gray-500 text-sm leading-relaxed">
        Tu solicitud fue enviada. Nuestro equipo revisara stock y te contactara
...
const RequestOrderPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Email GAS (fire-and-forget)
      sendGasEmail(payload, data.requestReference).catch((err) => {
        console.warn('[RequestOrder] GAS email fallido (no bloquea):', err);
      });
  ...
  return (
    <div className="container py-8 max-w-3xl">
      ...
      <p className="text-muted-foreground text-sm mb-6">
        Completa tus datos y nuestro equipo revisara stock y disponibilidad para
        habilitarte el pago.
      </p>
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        <p className="text-center text-xs text-gray-400 mt-3">
          Nuestro equipo revisara stock y te contactara para habilitar el pago.
          Sin cobro automatico.
        </p>
      </form>
    </div>
  );
