...
const CatalogPage = () => {
    ...
    const FilterPanel = () => (
          <div className="border rounded-xl p-4 space-y-5 sticky top-32">
                <div className="flex items-center justify-between">
                        <h3 className="font-semibold text-sm">Filtros</h3>h3>
                  {activeFilterCount > 0 && (
                ...
                <div>
                        <p className="text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wider">Categoria</p>p>
                        ...
                        <label key={cat.id} className="flex items-center gap-2 text-sm cursor-pointer hover:text-primary transition-colors">
                                      ...
                                      <span className="ml-auto text-xs text-muted-foreground">{cat.productCount}</span>span>
                        </label>label>
                      ))}
                        </div>div>
                </div>div>
                <div>
                        <p className="text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wider">Potencia</p>p>
                        <div className="space-y-1">
                          {wattageRanges.map(r => (
                ...
                <div>
                        <p className="text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wider">Temperatura de color</p>p>
                        <div className="space-y-1">
                          {kelvinOptions.map(k => (
                ...
                <div>
                        <p className="text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wider">Grado IP</p>p>
                        <div className="space-y-1">
                          {ipOptions.map(ip => (
    ...
    return (
          <div className="container py-8">
                <div className="mb-6">
                        <h1 className="text-2xl font-bold">{category?.name || 'Catalogo completo'}</h1>h1>
                ...
                <div className="flex flex-col lg:flex-row gap-6" ref={gridRef}>
                        <aside className="lg:w-64 shrink-0 hidden lg:block"><FilterPanel /></aside>aside>
                  ...
                  {filterOpen && (
                      <div className="lg:hidden fixed inset-0 z-50 flex">
                        ...
                        <div className="flex-1">
                                  <div className="flex items-center justify-between mb-4 gap-3">
                                              <Button variant="outline" size="sm" className="lg:hidden gap-1.5" onClick={() => setFilterOpen(true)}>
                                                            ...
                                                            Filtros
                                                {activeFilterCount > 0 && (
                            <span className="bg-primary text-primary-foreground text-[10px] rounded-full h-4 w-4 flex items-center justify-center">{activeFilterCount}</span>span>
                                                            )}
                                              </Button>Button>
                            ...
                            <span key={slug} className="inline-flex items-center gap-1 text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full">
                              {categories.find(c => c.slug === slug)?.name}
                                              <button onClick={() => setSelectedCategories(toggle(selectedCategories, slug))}><X className="h-3 w-3" /></button>button>
                            </span>span>
                          ))}
                                                {selectedWatts.map(w => (
                            <span key={w} className="inline-flex items-center gap-1 text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full">
                              {w}<button onClick={() => setSelectedWatts(toggle(selectedWatts, w))}><X className="h-3 w-3" /></button>button>
                            </span>span>
                          ))}
                            ...
                            <span key={k} className="inline-flex items-center gap-1 text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full">
                              {k}K<button onClick={() => setSelectedKelvin(toggle(selectedKelvin, k))}><X className="h-3 w-3" /></button>button>
                            </span>span>
                          ))}
                                                {selectedIP.map(ip => (
                            <span key={ip} className="inline-flex items-center gap-1 text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full">
                              {ip}<button onClick={() => setSelectedIP(toggle(selectedIP, ip))}><X className="h-3 w-3" /></button>button>
                            </span>span>
                          ))}
                                              </div>div>
                                              <div className="ml-auto">
                                                            <select
                                                                            ...
                                                                            >
                                                                            <option value="relevance">Relevancia</option>option>
                                                                            <option value="price-asc">Precio: menor a mayor</option>option>
                                                                            <option value="price-desc">Precio: mayor a menor</option>option>
                                                                            <option value="watts">Potencia</option>option>
                                                            </select>select>
                                              </div>div>
                                  </div>div>
                        ...
                        <div className="text-center py-16">
                                      <p className="text-muted-foreground mb-3">No se encontraron productos con los filtros seleccionados.</p>p>
                                      <button onClick={clearAll} className="text-sm text-primary hover:underline">Limpiar filtros</button>button>
                        </div>div>
                                  )}
                        ...
                        <div className="mt-8 flex flex-col items-center gap-3">
                                      ...
                                      <div className="flex items-center gap-1 flex-wrap justify-center">
                                                      <Button variant="outline" size="sm" className="gap-1 h-8 px-3" onClick={() => goToPage(safePage - 1)} disabled={safePage === 1}>
                                                                        ...
                                                                        <span className="hidden sm:inline">Anterior</span>span>
                                                      </Button>Button>
                                        {pageNumbers().map((n, i) =>
                                            n === '...' ? (
                                                                  <span key={`ellipsis-${i}`} className="px-2 text-muted-foreground text-sm select-none">&#8230;</span>span>
                                                                ) : (
                                                                  <Button
                                                                                          key={n}
                                                      ...
                                                      <Button variant="outline" size="sm" className="gap-1 h-8 px-3" onClick={() => goToPage(safePage + 1)} disabled={safePage === totalPages}>
                                                                        <span className="hidden sm:inline">Siguiente</span>span>
...
export default CatalogPage;</div>
