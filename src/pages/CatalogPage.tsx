        ...
        <div className="space-y-1">
          {categories.map(cat => (
            <label key={cat.id} className="flex items-center gap-2 text-sm cursor-pointer hover:text-primary transition-colors">
              <input type="checkbox" className="accent-primary" checked={selectedCategories.includes(cat.slug)} onChange={() => setSelectedCategories(toggle(selectedCategories, cat.slug))} />
              <span>{cat.name}</span>
        ...
        <div className="space-y-1">
          {wattageRanges.map(r => (
            <label key={r.label} className="flex items-center gap-2 text-sm cursor-pointer hover:text-primary transition-colors">
              <input type="checkbox" className="accent-primary" checked={selectedWatts.includes(r.label)} onChange={() => setSelectedWatts(toggle(selectedWatts, r.label))} />
              {r.label}
            </label>
          ))}
        </div>
        ...
        <div className="space-y-1">
          {kelvinOptions.map(k => (
            <label key={k} className="flex items-center gap-2 text-sm cursor-pointer hover:text-primary transition-colors">
              <input type="checkbox" className="accent-primary" checked={selectedKelvin.includes(k)} onChange={() => setSelectedKelvin(toggle(selectedKelvin, k))} />
              {k}K
            </label>
          ))}
        </div>
        ...
        <div className="space-y-1">
          {ipOptions.map(ip => (
            <label key={ip} className="flex items-center gap-2 text-sm cursor-pointer hover:text-primary transition-colors">
              <input type="checkbox" className="accent-primary" checked={selectedIP.includes(ip)} onChange={() => setSelectedIP(toggle(selectedIP, ip))} />
              {ip}
            </label>
          ))}
        </div>
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between mb-4 gap-3">
            ...
            <div className="ml-auto">
              <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="text-sm border rounded-lg px-3 py-2 bg-background focus:outline-none focus:ring-2 focus:ring-primary/30">
