...
function buildRepregunta1(missing: string[], captured: CapturedFields = {}): string {
          // Construir reconocimiento de contexto si hay info ya capturada
  const hints: string[] = [];
          if (captured.proyecto_o_uso) hints.push('el ' + captured.proyecto_o_uso);
          if (captured.tipo_de_luz) hints.push('luz ' + captured.tipo_de_luz);

  const intro =
              hints.length > 0
              ? 'Entendido, para ' + hints.join(' con ') + ' podemos orientarte bien.'
