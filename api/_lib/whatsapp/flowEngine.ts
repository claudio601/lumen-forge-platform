...
// [FIX-B] Intención comercial: excluye falsos wantsHuman de Claude
const COMMERCIAL_INTENT_RE = /\b(cotizar|cotizaci[oó]n|comprar|necesito|busco|quiero|requiero|presupuesto|precio|cuanto|cuánto)\b/i;
...
function looksLikeNewRequest(body: string): boolean {
            ...
            // [FIX-A] Intención comercial sola (necesito/busco/quiero) — reset aunque no haya producto+número
    if (NEW_REQ_INTENT_RE.test(b) && b.length > 10) return true;
            return false;
}
...
export function processFlowStep(
    ...
    // [FIX-C] Si hay intención comercial, claudeParsed.wantsHuman se ignora
    // Solo WANTS_HUMAN_RE y signals.wantsHuman son fuentes válidas de wantsHuman
    const hasCommercialIntent = COMMERCIAL_INTENT_RE.test(body);
            const claudeWantsHuman = hasCommercialIntent ? false : Boolean(claudeParsed.wantsHuman);
            const wantsHuman = state.wantsHuman || signals.wantsHuman || claudeWantsHuman || WANTS_HUMAN_RE.test(body);
    ...
    // PASO 2: wantsHuman — cierre inmediato con mensaje dedicado
    if (wantsHuman) {
