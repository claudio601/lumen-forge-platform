...
export function processFlowStep(
  ...
  if (claudeParsed.producto) merged.producto = claudeParsed.producto;
  ...
  while (currentStage !== 'closed') {
        if (currentStage === 'stage1') {
                ...
                if (repreg1 >= 1) {
                          ...
                          saveFlowState(phone, { ...state, stage: 'closed', captured: merged, captureStatus: finalStatus, repreguntasStage1: repreg1, repreguntasStage2: repreg2, repreguntasStage3: repreg3, wantsHuman });
                ...
                repreg1 += 1;
                saveFlowState(phone, { ...state, stage: 'stage1', captured: merged, repreguntasStage1: repreg1, repreguntasStage2: repreg2, repreguntasStage3: repreg3, wantsHuman });
      ...
      if (currentStage === 'stage2') {
              ...
              repreg2 += 1;
              saveFlowState(phone, { ...state, stage: 'stage2', captured: merged, repreguntasStage1: repreg1, repreguntasStage2: repreg2, repreguntasStage3: repreg3, wantsHuman });
      ...
      if (currentStage === 'stage3') {
              const missing = missingStage3(merged);
              if (missing.length === 0) {
                        const cs = computeCaptureStatus(merged);
                        saveFlowState(phone, { ...state, stage: 'closed', captured: merged, captureStatus: cs, repreguntasStage1: repreg1, repreguntasStage2: repreg2, repreguntasStage3: repreg3, wantsHuman });
              ...
              repreg3 += 1;
              saveFlowState(phone, { ...state, stage: 'stage3', captured: merged, repreguntasStage1: repreg1, repreguntasStage2: repreg2, repreguntasStage3: repreg3, wantsHuman });
              const replyS3 = missing.length === 2 ? MSG3 : buildRepregunta3(missing);
  ...
  return { reply: MSG4, shouldCreateDeal: false, shouldNotify: false, captureStatus: state.captureStatus, captured: merged, wantsHuman, closedFlow: true };
}
...
function buildRepregunta1(missing: string[]): string {
    if (missing.length === 1)
          return 'Gracias. Para continuar, indicame por favor ' + missing[0] + '.';
    return 'Gracias. Para continuar, indicame por favor:\n' + missing.map((x) => '- ' + x).join('\n') + '\n\nMe los puedes indicar?';
}

function buildRepregunta2(missing: string[]): string {
    if (missing.length === 1)
          return 'Solo me falta saber: ' + missing[0] + '. Me lo puedes indicar?';
    return 'Solo me falta saber:\n' + missing.map((x) => '- ' + x).join('\n') + '\n\nMe los puedes indicar?';
}

function buildRepregunta3(missing: string[]): string {
    if (missing.length === 1)
          return 'Para registrar tu solicitud necesito tu ' + missing[0] + '.';
