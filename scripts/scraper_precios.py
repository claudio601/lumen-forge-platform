    ...
    for category, base_url in TECNO_CATEGORIES.items():
        ...
        for page in range(1, TECNO_MAX_PAGES + 1):
            ...
            try:
                ...
                if not items:
                    logger.info("Tecno %s p%s: no products found.", category, page)
                    break
                for item in items:
...
def scrape_all() -> Dict[str, List[ProductPrice]]:
    ...
    if SCRAPER_MODE == "all":
        ...
        logger.info("Modo auto: omitiendo Megabright y TecnoIluminacion (requieren IP residencial)")
        # Crear entradas vacias para que aparezcan en el reporte
        scraped_at = datetime.now(timezone.utc).isoformat()
        for cat in ["Proyectores LED", "Paneles LED", "Campanas LED", "Tubos LED"]:
...
def generate_markdown(payload: dict) -> str:
    ...
    mode = payload.get("scraper_mode", "all")
    ...
    # -- Nota sobre competidores bloqueados --
    md.append("## Nota importante\n")
    md.append("> **Megabright** y **TecnoIluminacion** bloquean conexiones desde IPs de data center")
    md.append("> (Cloudflare WAF y bloqueo TCP respectivamente). Sus datos solo se actualizan")
    md.append("> mediante ejecucion manual del scraper desde una IP residencial:")
    md.append("> ")
    md.append('> ```bash')
    md.append("> SCRAPER_MODE=all python scripts/scraper_precios.py")
    md.append('> ```')
    md.append("> ")
    md.append("> La ejecucion automatica quincenal (GitHub Actions) solo captura datos de **PowerEnergy**.\n")

    md.append("## Resumen\n")
    md.append("| Categoria | Competidor | Min | Max | Productos |")
    md.append("|---|---|---:|---:|---:|")
    for cat in cats:
        for comp in comps:
